...
class WindowAttention(nn.Module):
    def __init__(self, dim, idx, split_size=[8, 8], dim_out=None, num_heads=6, attn_drop=0., proj_drop=0.,
        ...
        if self.position_bias:
            ...
            biases = torch.stack(torch.meshgrid(position_bias_h, position_bias_w, indexing='xy'))
...
class Block(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        res = x
        attn_output = self.drop_path(self.attn(self.norm1(x), H, W))
        attn_output = attn_output.view_as(x)
...
class DMlp(nn.Module):
    def __init__(self, num_features, growth_rate=2.0):
        super().__init__()
        hidden_dim = int(num_features * growth_rate)
        self.conv_0 = nn.Sequential(
            nn.Conv2d(num_features,hidden_dim,3,1,1,groups=num_features),
            nn.Conv2d(hidden_dim,hidden_dim,1,1,0)
        )
        self.act =nn.GELU()
        self.conv_1 = nn.Conv2d(hidden_dim, num_features, 1, 1, 0)
...
class SMFA(nn.Module):
    def __init__(self, num_features=36):
        ...
        self.linear_0 = nn.Conv2d(num_features,num_features*2,1,1,0)
        self.linear_1 = nn.Conv2d(num_features,num_features,1,1,0)
        self.linear_2 = nn.Conv2d(num_features,num_features,1,1,0)

        self.lde = DMlp(num_features,2)

        self.dw_conv = nn.Conv2d(num_features,num_features,3,1,1,groups=num_features)

        self.gelu = nn.GELU()
        self.down_scale = 8

        self.alpha = nn.Parameter(torch.ones((1,num_features,1,1)))
        self.belt = nn.Parameter(torch.zeros((1,num_features,1,1)))
...
class PCFN(nn.Module):
    def __init__(self, num_features, growth_rate=2.0, p_rate=0.25):
        super().__init__()
        hidden_dim = int(num_features * growth_rate)
        p_dim = int(hidden_dim * p_rate)
        self.conv_0 = nn.Conv2d(num_features,hidden_dim,1,1,0)
        self.conv_1 = nn.Conv2d(p_dim, p_dim ,3,1,1)

        self.act =nn.GELU()
        self.conv_2 = nn.Conv2d(hidden_dim, num_features, 1, 1, 0)
...
class FMB(nn.Module):
    def __init__(self, num_features, ffn_scale=2.0):
        super().__init__()

        self.smfa = SMFA(num_features)
        self.pcfn = PCFN(num_features, ffn_scale)
...
class PlantSR(nn.Module):
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 split_size=[8,8],
                 c_ratio=0.5,ffn_scale=2,n_blocks=5):
        ...
        self.num_layers = len(depth)
        self.use_chk = use_chk
        self.num_features = self.embed_dim = num_features  # num_features for consistency with other models
        heads = num_heads
        ...
        for i in range(self.num_layers):
            ...
            if resi_connection == '1conv':
                ...
                self.conv_after_body = nn.Sequential(
                    nn.Conv2d(num_features, num_features // 4, 3, 1, 1), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                    nn.Conv2d(num_features // 4,num_features // 4, 1, 1, 0), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                    nn.Conv2d(num_features // 4, num_features, 3, 1, 1))

        # deep feature extraction
        self.deep = nn.Sequential(*[FMB(num_features, ffn_scale) for _ in range(n_blocks)])
    ...
    def forward(self, x):
        ...
        # Middle feature
        m = self.conv_after_body(self.forward_features(x)) + x
